...
1) Create + activate venv (if you haven't already):
...
2) Force the dummy LLM (no network calls) — useful for offline testing and deterministic output:

```powershell
Remove-Item Env:\\OPENAI_API_KEY -ErrorAction SilentlyContinue
```

3) Run the example script which uses SAMPLE_RESUME and SAMPLE_TRANSCRIPT:
